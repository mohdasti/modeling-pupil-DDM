# R/fit_primary_vza_biasintx.R

# Primary v+z+a model with:
# 1. Bias interaction: difficulty × task (targets accuracy patterns)
...
LOG_FILE <- file.path(LOG_DIR, "fit_primary_vza_biasintx.log")
...
log_line("START fit_primary_vza_biasintx.R")
...
# 1) drift: keep random intercept only (simpler than random slopes)

# 2) boundary: keep difficulty + task main effects

# 3) ndt: small condition effects, no RE (stable)

# 4) bias: add difficulty × task interaction (targets accuracy patterns)

# NOTE: Removed random slopes on drift to avoid initialization/sampling issues
...
pri <- c(
  ...
  prior(normal(0, 0.35),            class="b", dpar="bs"),              # bs slopes

  prior(normal(0, 0.25),            class="b", dpar="bias"),            # tighter bias slopes

  prior(normal(0, 0.08),            class="b", dpar="ndt"),             # tiny ndt condition effects

  prior(student_t(3, 0, 0.30),      class="sd")

  # Note: No lkj prior needed - removed random slopes, so no correlation matrix
...
fit_file <- file.path(OUT_DIR, "primary_vza_biasintx")

log_line("Beginning fit: primary_vza_biasintx")
...
log_line("Model features: bias interaction (difficulty × task), random intercept on drift")
...
saveRDS(fit, file.path(PUBLISH_DIR, "fit_primary_vza_biasintx.rds"))

cat("✓ primary v+z+a (bias interaction + drift RE slope) saved at: ", 
...
log_line("END fit_primary_vza_biasintx.R")

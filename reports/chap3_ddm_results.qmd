...
fig_path <- function(fig_name) {
  ...
  # HTML: prefer PNG/JPG (better browser compatibility)
  ...
  return(file.path("../output/figures", if (!is.null(png_abs)) png_name else if (!is.null(jpg_abs)) jpg_name else pdf_name))
}
...
for (path in pupil_data_paths) {
  if (file.exists(path)) {
    tryCatch({
      ...
      cat("✓ Loaded pupil data from:", path, "\n")
      cat("  Rows:", nrow(pupil_data), "\n")
      cat("  Columns:", length(names(pupil_data)), "\n")
      break
    }, error = function(e) {
      cat("Error loading pupil data from", path, ":", e$message, "\n")
      NULL
    })
  }
}
...
if (is.null(pupil_data)) {
  cat("⚠ WARNING: pupil_data is NULL. Tables 12, 13, and 14 will show placeholders.\n")
  cat("  Checked paths:\n")
  for (path in pupil_data_paths) {
    cat("    ", path, ":", file.exists(path), "\n")
  }
} else {
  cat("✓ pupil_data loaded successfully with", nrow(pupil_data), "rows\n")
  cat("  Available columns:", paste(head(names(pupil_data), 10), collapse=", "), "...\n")
}
...
#| fig-align: center
#| out-width: "90%"
knitr::include_graphics(fig_path("ddm_pipeline"))
```
...
#| fig-align: center
#| out-width: "90%"
knitr::include_graphics(fig_path("AUC_timeline.jpg"))
```
...
#| fig-align: center
#| out-width: "90%"
knitr::include_graphics(fig_path("matrix.png"))
```
...
if (!is.null(publish_gate)) {
  # Simpler, more robust table: show the convergence/PPC gate values as-is
  publish_gate %>%
    gt()
} else {
...
if (!is.null(pupil_data)) {
  ...
  if (file.exists(waveform_file)) {
    ...
    if (nrow(waveform_ch3) > 0) {
      ...
      cat("TEPR time course figure: Waveform data not available for chapter 3.\n")
    }
  } else {
    # Placeholder if file not found
    cat("TEPR time course figure: Waveform summary file not found at", waveform_file, "\n")
  }
} else {
  # Placeholder if pupil_data not loaded
  cat("TEPR time course figure: Pupil data not loaded. Please ensure pupil data files are available.\n")
}
```
...
#| fig-align: center
#| out-width: "90%"
knitr::include_graphics(fig_path("pupil_ddm_scatter_phasicW3_bs.png"))
```
...
#| fig-align: center
#| out-width: "90%"
knitr::include_graphics(fig_path("pupil_ddm_scatter_phasicW1p3_v.png"))
```
...
#| fig-align: center
#| out-width: "90%"
knitr::include_graphics(fig_path("pupil_ddm_scatter_tonic_v.png"))
```
...
if (knitr::is_latex_output()) {
  ...
  knitr::include_graphics(fig_path("pupil_ddm_robustness.png"))
...
lc_fig_alt_pdf <- file.path(proj_root, "output", "figures", "fig_lc_integrity_cnr_vs_delta_z.pdf")
lc_fig_alt_png <- file.path(proj_root, "output", "figures", "fig_lc_integrity_cnr_vs_delta_z.png")

# Try multiple paths
fig_found <- FALSE
if (file.exists(lc_fig_file)) {
  knitr::include_graphics(lc_fig_file)
  fig_found <- TRUE
} else if (file.exists(lc_fig_alt_pdf)) {
  knitr::include_graphics(lc_fig_alt_pdf)
  fig_found <- TRUE
} else if (file.exists(lc_fig_alt_png)) {
  knitr::include_graphics(lc_fig_alt_png)
  fig_found <- TRUE
}
...
lc_fig_alt2_pdf <- file.path(proj_root, "output", "figures", "fig_lc_integrity_pc1_vs_ddm_v.pdf")
lc_fig_alt2_png <- file.path(proj_root, "output", "figures", "fig_lc_integrity_pc1_vs_ddm_v.png")

# Try multiple paths
if (file.exists(lc_fig_file2)) {
  knitr::include_graphics(lc_fig_file2)
} else if (file.exists(lc_fig_alt2_pdf)) {
  knitr::include_graphics(lc_fig_alt2_pdf)
} else if (file.exists(lc_fig_alt2_png)) {
  knitr::include_graphics(lc_fig_alt2_png)
}
# If no figure found, chunk returns nothing (no NULL)
```
...
lc_fig_alt3_pdf <- file.path(proj_root, "output", "figures", "fig_lc_integrity_pc1_vs_pupil_phasic.pdf")
lc_fig_alt3_png <- file.path(proj_root, "output", "figures", "fig_lc_integrity_pc1_vs_pupil_phasic.png")

# Try multiple paths
if (file.exists(lc_fig_file3)) {
  knitr::include_graphics(lc_fig_file3)
} else if (file.exists(lc_fig_alt3_pdf)) {
  knitr::include_graphics(lc_fig_alt3_pdf)
} else if (file.exists(lc_fig_alt3_png)) {
  knitr::include_graphics(lc_fig_alt3_png)
}
# If no figure found, chunk returns nothing (no NULL)
```
...
The hierarchical structure of our model reveals substantial individual differences in all DDM parameters. Drift rate (v) shows the widest variability, consistent with the heterogeneity in evidence accumulation speed observed in aging populations. Boundary separation (a) and bias (z) also show meaningful individual variation, supporting the use of hierarchical modeling to account for between-subject differences. Parameter correlations reveal a weak negative relationship (r = -0.205) between drift intercept and bias intercept at the subject level, suggesting that participants who show stronger negative drift tend to have less bias toward "Different" responses. The Brinley plot reveals a strong positive relationship between Easy and Hard RTs, with a slope > 1 indicating disproportionate slowing in Hard trials—a hallmark of generalized slowing in older adults. Supporting figures are provided in Supplementary Figures @fig-subject-parameter-distribution–@fig-brinley-plot.
...
**PPC Summary (Joint Model)**: PPCs were good for Standard and Easy cells (QP RMSE \< 0.13, KS \< 0.08), with modest misfit in VDT-Hard (worst QP RMSE ≈ 0.206). This pattern suggests some residual fast-tail behavior not captured by a constant-drift Wiener process. Cell-wise PPC residuals are shown in Supplementary Figure @fig-pdiff-heatmap. Additional PPC diagnostics (best/median/worst cells) are shown in Supplementary Figure @fig-ppc-small-multiples.
...
To validate the internal consistency of our model estimates, we performed three sanity checks recommended by independent expert review. These checks verify that parameter estimates are mathematically consistent with observed behavioral patterns. All sanity-check figures are provided in Supplementary Figures @fig-sanity-rt-asymmetry–@fig-sanity-subject-heterogeneity.
...
RT overlay PPCs are shown in Supplementary Figure @fig-ppc-rt-overlay. QP plot diagnostics are shown in Supplementary Figure @fig-qp-plot.

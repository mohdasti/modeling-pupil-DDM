...
4. **Starting bias ($z$)** may shift toward 0.5 under high effort if strong phasic LC–NE responses reduce pre-existing response tendencies (i.e., a bias “reset”) [@degee2020pupil]. 
   ...
   **Exploratory:** If LC structural integrity is available, we will explore whether individual differences in LC integrity moderate the magnitude of any effort-related shift in $z$ (higher integrity → stronger/cleaner bias reset), consistent with accounts linking LC integrity to arousal regulation in aging [@huang2024; @mather2016].
...
LC MRI metrics were computed by colleagues in our research group; I did not compute them for this dissertation chapter. The methodological approach follows [@bennett2024lc], who describe a pipeline for deriving LC contrast from magnetization transfer (MT) imaging and extracting diffusion metrics within LC regions of interest (ROIs). Specifically, [@bennett2024lc] compute an MTC (magnetization transfer contrast) map relative to a pontine reference region and extract LC ROI summary values (average/maximum contrast). Diffusion metrics (DTI/NODDI-style metrics) were extracted within the LC ROI following the diffusion preprocessing approach described in [@bennett2024lc].

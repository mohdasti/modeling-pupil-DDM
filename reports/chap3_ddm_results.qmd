...
**Summary**: Pupillometry data preprocessing is complete (see Methods section). Descriptive statistics and TEPR quantification are reported above: (1) data quality metrics (percentage of valid samples, artifact exclusion rates), (2) baseline pupil diameter (tonic arousal) by task and effort condition, and (3) task-evoked pupil response (TEPR) quantified as cognitive AUC by condition. Baseline pupil diameter reflects tonic LC-NE activity, while cognitive AUC captures phasic arousal during the decision period. These measures allow us to link physiological arousal states to the latent decision processes identified by the DDM. Future analyses will explore correlations between pupil measures and DDM parameters to test mechanistic hypotheses about how arousal modulates decision-making.

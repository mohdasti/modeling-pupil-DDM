...
```{r, fig.cap="DDM Parameter Correlation Matrix. Correlation coefficients between posterior estimates of DDM parameter intercepts. Positive correlations indicate parameters that tend to increase together; negative correlations indicate trade-offs. The diagonal shows perfect correlation (1.0) for each parameter with itself. This matrix helps identify whether participants who show high drift rates also show high boundary separation (speed-accuracy tradeoff), or whether bias is independent of other parameters.", out.width="85%", fig.align="center"}
knitr::include_graphics(fig_path("fig_parameter_correlation_matrix.pdf"))
```

**Interpretation**: The correlation matrix reveals the structure of relationships between DDM parameters. Strong correlations may indicate shared underlying mechanisms (e.g., arousal affecting multiple parameters simultaneously), while weak correlations suggest independent cognitive processes. These relationships inform our understanding of how effort and difficulty modulate decision-making components.

...
Grounded in the behavioral and physiological frameworks above, we test four primary predictions; additionally, we conduct an exploratory analysis examining whether LC structural integrity moderates the predicted bias shift.
...
4. **Starting bias ($z$)** may shift toward 0.5 under high effort if strong phasic LC–NE responses reduce pre-existing response tendencies (i.e., a bias “reset”) [@degee2020pupil]. 
   ...
   **Exploratory:** We will explore whether individual differences in LC integrity moderate the magnitude of any effort-related shift in $z$ (higher integrity → stronger/cleaner bias reset), consistent with accounts linking LC integrity to arousal regulation in aging [@huang2024; @mather2016].
...
67 older adults (≥65 years; mean = 71.3, SD = 4.8). This analysis uses the same dataset and participants as described in the LC behavioral report manuscript (see References). All participants provided informed consent and received course credit or financial compensation for participation. Study procedures were approved by the Institutional Review Board of the University of California, Riverside and all experimental procedures were performed in accordance with the approved guidelines and regulations.
...
**Confounds and sensitivity checks.** To address potential concerns about post-response contamination and motor execution confounds, we implemented multiple mitigation strategies. First, we computed an early cognitive window (W1.3: target+0.3 to target+1.3s; 4.65s to 5.65s relative to squeeze onset) that extends only 0.95s into the response period. This window yields two complementary metrics: `cog_auc_w1p3` (area under the curve in the W1.3 window) and `cog_mean_w1p3` (mean pupil diameter in the W1.3 window). These early-window features provide a more conservative estimate of pre-response cognitive processing, as they capture the initial phase of the TEPR with minimal contamination from post-response processes. Second, for trial-level DDM-pupil coupling models (when implemented), RT should be included as a covariate to control for decision state and prevent RT from confounding pupil-DDM relationships. This is critical because RT correlates with both decision processes and pupil dynamics during the response period. Note: The current analyses use subject-level correlations between DDM parameters and pupil measures, which aggregate across trials and thus naturally account for RT variability at the subject level. Third, window definitions are computed to truncate W3.0 at 150ms before button press (`t_resp - 0.15s`) to minimize motor execution/blink contamination (available for future analyses; full AUC computation for motor-buffered windows requires re-processing flat files with RT information). While our primary analyses focus on the full TEPR magnitude (W3.0 window), the W1.3 features serve as robustness checks to ensure that observed relationships between cognitive arousal and DDM parameters are not solely driven by post-response contamination. Both `cog_auc_w1p3` and `cog_mean_w1p3` are included in exploratory analyses and sensitivity checks alongside the primary W3.0 metric.

## Exploratory: Locus Coeruleus (LC) Integrity Metrics

LC MRI metrics were precomputed by colleagues in our research group and are used here only for exploratory individual-differences analyses. The methodological approach follows [@bennett2024lc], who describe a pipeline for deriving LC contrast from magnetization transfer (MT) imaging and extracting diffusion metrics within LC regions of interest (ROIs). Specifically, [@bennett2024lc] compute an MTC (magnetization transfer contrast) map relative to a pontine reference region and extract LC ROI summary values (average/maximum contrast). Diffusion metrics (DTI/NODDI-style metrics) were extracted within the LC ROI following the diffusion preprocessing approach described in [@bennett2024lc].

The available columns from the LC metrics dataset (`LC_Grant_Updated_LC_values.2026.csv`) include:

- **LC CNR** (LC contrast-to-noise ratio): Treated as an LC contrast proxy, reflecting MT-weighted signal intensity within the LC ROI relative to background noise
- **LC Vol** (LC volume): Structural volume measure of the LC ROI
- **DTI metrics**: Fractional anisotropy (FA) and mean diffusivity (MD), reflecting white matter microstructural properties within the LC ROI
- **NODDI metrics**: Free water fraction (fiso) and intracellular volume fraction (ficvf), providing complementary microstructural indices of LC tissue organization

These LC integrity metrics are used in exploratory individual-differences analyses to examine whether LC structural properties relate to (i) subject-level pupil indices (baseline diameter, TEPR magnitude), and (ii) subject-level DDM parameters and effort-related shifts (especially bias shift), as exploratory moderation/association analyses. These analyses are explicitly exploratory and are not part of the primary confirmatory hypothesis testing framework. These measures are not used to select models or adjudicate primary hypotheses.
...
**Priors**: All priors are weakly informative and set on the link scale. Intercepts: v Intercept ~ Normal(0, 1); bs Intercept ~ Normal(log(1.7), 0.30) → a ≈ 1.7 on natural scale; ndt Intercept ~ Normal(log(0.23), 0.12) → t₀ ≈ 230 ms on natural scale; bias Intercept ~ Normal(0, 0.5) → z ≈ 0.5 (no bias) on probability scale. Slopes: v slopes: Normal(0, 0.6–0.7); bs slopes: Normal(0, 0.25–0.30); bias slopes: Normal(0, 0.35). Random effects: Standard deviations: Student-t(3, 0, 0.30); Correlations: LKJ(2). Prior vs. posterior for t0 is shown in Supplementary Figure @fig-ndt-prior-posterior. The posterior for t₀ is well-informed by the data while remaining compatible with the weakly informative prior, confirming adequate identifiability for the group-level intercept despite the response-signal design.

**MCMC sampling**: Algorithm: NUTS (No-U-Turn Sampler). Chains: 4. Iterations: 8,000 per chain (4,000 warmup, 4,000 sampling). Convergence criteria: $\hat{R}$ ≤ 1.01 [@gelman1992inference; @vehtari2021rank], minimum bulk/tail ESS ≥ 400. Sampling controls: NUTS with `adapt_delta = 0.995`, `max_treedepth = 15`.

**Model comparison**: We compared 10 candidate models varying in how difficulty, task, and effort map onto DDM parameters. Leave-one-out cross-validation (LOO-CV) was used to select the best-fitting model. @tbl-loo-comparison shows the LOO-CV results for all candidate models.
...
if (!is.null(loo_primary)) {
  ...
  # Only apply cell coloring for HTML/DOCX, not PDF (avoids LaTeX colortbl/booktabs conflicts)
  if (!knitr::is_latex_output()) {
    loo_table <- loo_table %>%
      tab_style(
...
The model with **difficulty → (v + a)** is strongly favored, with bias constrained to be constant across difficulty levels (ΔELPD vs. v-only: ≈ +185, SE ≈ 20; stacking weight: ≈ 0.89; PBMA weight: ≈ 1.0). Pareto-k diagnostics: 1/17,834 observations had k > 0.7; moment matching was not required. The full LOO comparison plot is provided in Supplementary Figure @fig-loo. The data strongly support a model in which task difficulty modulates drift rate and boundary separation. Starting-point bias is constrained to be constant across difficulty levels (as trials are randomized), but varies by task and effort. Simpler models (e.g., difficulty affecting only drift) are decisively rejected by cross-validation. Detailed posterior predictive checks and visual diagnostics are provided in Appendix A3 (see @sec-ch3-ppc-method–@sec-ch3-ppc-visual).

**Convergence diagnostics**: @tbl-convergence-ppc-gate shows convergence diagnostics and PPC gate results.
...
**Result**: The primary model passes all MCMC convergence gates ($\hat{R}$, ESS, divergent transitions). Detailed PPC methods, primary gate results, and visual diagnostics are provided in Appendix A3 (see @sec-ch3-ppc-method–@sec-ch3-ppc-visual; @tbl-ch3app-ppc-subj-summary and @fig-ch3app-ppc-small-multiples).

# Results

## Results Overview

Our hierarchical drift-diffusion model analysis of 17,834 trials from 67 older adults revealed several key findings. First, on Standard (Δ=0) trials, participants showed a strong conservative response tendency (89.1% chose "same"), driven primarily by negative drift rate (v ≈ -1.26) rather than starting-point bias, indicating active evidence accumulation toward "same" responses when stimuli are identical. Second, task difficulty dominated drift rate effects: Easy trials showed substantially faster evidence accumulation than Hard trials (Δ ≈ +1.50 units/s), with difficulty also modulating boundary separation. Third, high physical effort (40% MVC) produced small but credible effects: drift rate decreased (consistent with resource competition) and non-decision time increased (consistent with cognitive-motor interference), but boundary separation did not increase as predicted by strategic adaptation accounts. Fourth, pupillometry-DDM integration revealed a negative correlation between phasic arousal and drift rate (r = -0.192, 95% CrI [-0.250, -0.134]), supporting supra-optimal arousal accounts, while the boundary separation correlation was minimal (r ≈ 0.04). Fifth, model fit was acceptable overall: subject-wise mid-body posterior predictive checks (PPCs) showed ≤15% of cells flagged, with localized misfit primarily in Easy/VDT fast-tail responses (see Appendix A3, @sec-ch3-ppc-method–@sec-ch3-ppc-visual for detailed PPC diagnostics). Sixth, exploratory LC integrity analyses revealed no strong moderation of effort effects by LC structural properties. The model converged well (max $\hat{R}$ ≤ 1.01, ESS ≥ 400) and was strongly favored by leave-one-out cross-validation (ΔELPD ≈ +185 vs. drift-only model).

### Descriptive: Response Tendency on Standard Trials

On Standard (Δ=0) trials, participants chose "same" on 89.1% of trials and "different" on 10.9%—consistent with a conservative response tendency. While Standard trials theoretically have zero objective evidence difference (Δ=0), our model estimated a strong negative drift rate (v ≈ -1.26 in the primary model), indicating that participants actively accumulate evidence toward "same" responses when stimuli are identical. The observed preference for "same" responses reflects the combined effects of both drift and starting-point bias, with drift dominating the decision process. This bias estimate would be unobtainable using accuracy-coded models, where Standard trials would be ambiguous (both "same" and "different" responses are technically correct when stimuli are identical).

## Sample and Data Quality

Sample demographics and trial counts are shown in @tbl-demographics and @tbl-trial-counts.
...
Accuracy descriptives are shown in @tbl-accuracy-by-condition.
...
Reaction time descriptives are shown in @tbl-global-rt and @tbl-rt-by-condition. @fig-rt-distribution shows RT distributions by condition.
...
Pupillometry data quality metrics are shown in @tbl-pupil-quality.

```{r}
#| label: tbl-pupil-quality
#| tbl-cap: "Pupillometry Data Quality Summary"
#| tbl-pos: "htbp"
# Compute pupil data quality metrics
if (!is.null(pupil_data)) {
  # Compute quality metrics
  n_subj_pupil <- length(unique(pupil_data$subject_id))
  
  # Quality proportions (if available)
  quality_cols <- c("baseline_quality", "overall_quality", "cog_quality")
  quality_col <- intersect(quality_cols, names(pupil_data))[1]
  
  if (!is.null(quality_col) && !is.na(quality_col)) {
    mean_quality_trial <- mean(pupil_data[[quality_col]], na.rm = TRUE) * 100
    sd_quality_trial <- sd(pupil_data[[quality_col]], na.rm = TRUE) * 100
    
    # Subject-level mean quality
  ...
  pupil_quality_placeholder %>%
    ...
    tab_footnote(
      footnote = "Pupil data file not found. Please ensure ch3_triallevel.csv is available in quick_share_v7/analysis_ready/",
      locations = cells_body()
    )
}
```
...
Task-evoked pupil response (TEPR) by condition is shown in @tbl-tepr, and @fig-tepr-timecourse shows the TEPR time course.
...
**Summary**: Pupillometry preprocessing is complete (see Methods). We now present analyses examining correlations between pupil measures and DDM parameters to test mechanistic hypotheses about how arousal modulates decision-making (see @sec-pupil-ddm-integration).

## Pupil-DDM Integration {#sec-pupil-ddm-integration}

### Background and Hypotheses

Pupillometry provides a non-invasive proxy for Locus Coeruleus-Norepinephrine (LC-NE) system activity, with baseline pupil diameter reflecting tonic arousal and task-evoked pupil responses (TEPR) reflecting phasic arousal. We examined relationships between these pupillometry measures and individual differences in DDM parameters to test mechanistic hypotheses: (1) higher phasic arousal may relate to increased boundary separation, reflecting increased caution under arousal (the "hold your horses" account; [@cavanagh2014]); (2) higher arousal (both tonic and phasic) may relate to reduced drift rates, consistent with supra-optimal arousal or resource competition accounts in older adults; (3) phasic arousal may suppress pre-existing choice biases, resetting the decision process toward neutrality [@degee2020pupil].

For these analyses, we computed subject-level DDM parameter estimates as the posterior mean of subject-specific coefficients (fixed intercept + subject random effect) from the primary model. Subject-level pupil measures were aggregated as means across trials for each subject, with quality filters applied (see Methods). Correlations were computed using posterior draws: for each draw, we computed Pearson correlations between subject-level DDM parameters and subject-level pupil measures, then summarized the posterior distribution of correlation coefficients.

### Main Findings

#### Boundary separation (bs)

The relationship between phasic arousal and boundary separation was small but directionally positive: r = 0.043 [0.006, 0.079], P(r>0) = 0.990, n = 55 (@fig-pupil-ddm-scatter-phasicW3-bs). While the direction is credibly positive and consistent with the "hold your horses" account [@cavanagh2014], the effect size is minimal (r ≈ 0.04) and unlikely to be of practical significance.

#### Drift rate (v)

Both tonic and phasic arousal showed negative associations with drift rate. The strongest association was observed for phasic arousal in the early window (W1.3: target+0.3s → target+1.3s): r = -0.192 [-0.250, -0.134], P(r<0) = 1.000, n = 55 (@fig-pupil-ddm-scatter-phasicW1p3-v). Tonic arousal also showed a negative association: r = -0.140 [-0.200, -0.079], P(r<0) = 1.000, n = 55 (@fig-pupil-ddm-scatter-tonic-v). These negative associations suggest that higher arousal relates to reduced information accumulation efficiency, consistent with supra-optimal arousal or resource competition accounts.

#### Starting-point bias (z)

Associations with starting-point bias were weak and ambiguous. The tonic arousal × bias correlation was r = 0.085 [-0.021, 0.190], P(r>0) = 0.941, n = 55, with credible intervals overlapping zero. Phasic arousal measures showed similarly weak and inconsistent relationships with bias, with credible intervals spanning both positive and negative values. These findings suggest that arousal does not strongly modulate starting-point bias in this sample.

```{r}
#| label: fig-pupil-ddm-scatter-phasicW3-bs
#| fig-cap: "Phasic arousal (W3.0; cognitive AUC) versus boundary separation (bs). Each point shows a subject-level mean pupil response and the corresponding subject-level DDM estimate. The posterior Pearson correlation was small but directionally positive, r = 0.043 [0.006, 0.079], P(r>0) = 0.990, n = 55, consistent in direction with the 'hold your horses' account [@cavanagh2014] but minimal in magnitude."
#| fig-align: center
#| out-width: "90%"
include_graphics_safe(fig_path("pupil_ddm_scatter_phasicW3_bs.png"))
```

```{r}
#| label: fig-pupil-ddm-scatter-phasicW1p3-v
#| fig-cap: "Early phasic arousal (W1.3 window) versus drift rate (v). Higher early task-evoked pupil response was associated with lower drift rate, r = -0.192 [-0.250, -0.134], P(r<0) = 1.000, n = 55, consistent with an interpretation in which elevated arousal is linked to reduced evidence quality or increased internal noise in older adults."
#| fig-align: center
#| out-width: "90%"
include_graphics_safe(fig_path("pupil_ddm_scatter_phasicW1p3_v.png"))
```

```{r}
#| label: fig-pupil-ddm-scatter-tonic-v
#| fig-cap: "Tonic arousal (baseline pupil) versus drift rate (v). Larger baseline pupil diameter was associated with lower drift rate, r = -0.140 [-0.200, -0.079], P(r<0) = 1.000, n = 55."
#| fig-align: center
#| out-width: "90%"
include_graphics_safe(fig_path("pupil_ddm_scatter_tonic_v.png"))
```

```{r}
#| label: fig-pupil-ddm-robustness
#| fig-cap: ""
#| fig-align: center
#| out-width: "90%"
#| results: asis
if (knitr::is_latex_output()) {
  caption_text <- "Robustness checks for pupil–DDM correlations. Left: Pearson versus Spearman correlations show consistent directions across methods. Right: leave-one-out (LOO) sensitivity ranges indicate that the negative phasic W1.3 × v relationship remains negative across deletions, whereas the small positive phasic W3.0 × bs relationship is more sensitive to individual subjects."
  caption_text_escaped <- latex_escape(caption_text)
  knitr::asis_output(paste0(
    "\\begin{landscape}\n",
    "\\begin{figure}[H]\n",
    "\\centering\n",
    "\\includegraphics[width=0.9\\textwidth]{", fig_path("pupil_ddm_robustness.png"), "}\n",
  ...
  include_graphics_safe(fig_path("pupil_ddm_robustness.png"))
  cat("\n\n*Figure: Robustness checks for pupil–DDM correlations. Left: Pearson versus Spearman correlations show consistent directions across methods. Right: leave-one-out (LOO) sensitivity ranges indicate that the negative phasic W1.3 × v relationship remains negative across deletions, whereas the small positive phasic W3.0 × bs relationship is more sensitive to individual subjects.*\n")
}
```

### Robustness Checks

Results were robust across multiple sensitivity analyses (@fig-pupil-ddm-robustness). Pearson and Spearman correlations showed consistent patterns, with Spearman correlations generally showing similar or slightly stronger effects than Pearson correlations. Leave-one-subject-out (LOO) analysis confirmed that the drift findings were robust to individual subject removal: for phasic W1.3 vs v, the LOO range was [-0.240, -0.142], with the full-sample correlation of -0.192. The boundary separation effect, while small, was also stable across subjects (LOO range: [-0.003, 0.082]).

Trial-count sensitivity analyses (subsets with ≥50, ≥100, ≥150 trials) showed consistent patterns across thresholds (@tbl-pupil-ddm-trialcount-sensitivity). Weighted correlations using trial counts as weights yielded similar results to unweighted correlations. Task-stratified analyses (ADT vs VDT) showed similar patterns across tasks (@tbl-pupil-ddm-task-stratified), though with reduced power due to smaller sample sizes per task.

```{r}
#| label: tbl-pupil-ddm-trialcount-sensitivity
#| tbl-cap: "Trial-count sensitivity analysis for key pupil–DDM relationships. Correlations were recomputed under increasingly conservative minimum-trial thresholds to evaluate stability."
#| echo: false
#| message: false
#| warning: false
trial_sens <- sread(file.path(proj_root, "output/pupil_ddm/qc/pupil_ddm_trialcount_sensitivity.csv")) %||%
              sread("output/pupil_ddm/qc/pupil_ddm_trialcount_sensitivity.csv") %||%
              sread(file.path("..", "output/pupil_ddm/qc/pupil_ddm_trialcount_sensitivity.csv"))
if (!is.null(trial_sens) && nrow(trial_sens) > 0) {
  tbl_data <- trial_sens %>%
    dplyr::mutate(
      relationship = dplyr::case_when(
        relationship == "phasic_w3_bs" ~ "Phasic W3.0 × bs",
        relationship == "phasic_w1p3_v" ~ "Phasic W1.3 × v",
        relationship == "tonic_v" ~ "Tonic × v",
        TRUE ~ relationship
      ),
      r = round(r, 3),
    ...
    dplyr::select(relationship, min_trials, n_subjects, r, p_value, ci_lower, ci_upper)
  ...
  tbl <- tbl_data %>%
    ...
    gt::fmt_number(columns = c(r, p_value, ci_lower, ci_upper), decimals = 3) %>%
    gt::fmt_number(columns = c(min_trials, n_subjects), decimals = 0) %>%
    gt::tab_options(
  ...
  # Create placeholder table instead of text
  placeholder <- tibble(
    Relationship = "Data not available",
    `Min trials` = NA_integer_,
    N = NA_integer_,
    r = NA_real_,
    p = NA_real_,
    `CI lower` = NA_real_,
  ...
  placeholder %>%
    gt::gt() %>%
    gt::tab_footnote(
      footnote = "Trial-count sensitivity data not available. Please ensure the file output/pupil_ddm/qc/pupil_ddm_trialcount_sensitivity.csv exists.",
      locations = cells_body()
    )
}
```
...
```{r}
#| label: tbl-pupil-ddm-task-stratified
#| tbl-cap: "Task-stratified correlations (ADT vs. VDT) for key pupil–DDM relationships."
#| echo: false
#| message: false
#| warning: false
task_strat <- sread(file.path(proj_root, "output/pupil_ddm/qc/pupil_ddm_task_stratified.csv")) %||%
              sread("output/pupil_ddm/qc/pupil_ddm_task_stratified.csv") %||%
              sread(file.path("..", "output/pupil_ddm/qc/pupil_ddm_task_stratified.csv"))
if (!is.null(task_strat) && nrow(task_strat) > 0) {
  tbl_data <- task_strat %>%
    dplyr::mutate(
      relationship = dplyr::case_when(
        relationship == "phasic_w3_bs" ~ "Phasic W3.0 × bs",
    ...
    dplyr::select(relationship, task, n_subjects, r, p_value, ci_lower, ci_upper)
  ...
  tbl <- tbl_data %>%
    ...
    gt::fmt_number(columns = c(r, p_value, ci_lower, ci_upper), decimals = 3) %>%
    gt::fmt_number(columns = n_subjects, decimals = 0) %>%
    gt::tab_options(
      table_body.hlines.style = "none",
      table_body.vlines.style = "none",
      table_body.border.bottom.style = "none",
  ...
  placeholder %>%
    gt::gt() %>%
    gt::tab_footnote(
      footnote = "Task-stratified correlations data not available. Please ensure the file output/pupil_ddm/qc/pupil_ddm_task_stratified.csv exists.",
      locations = cells_body()
    )
}
```
...
The negative relationship between arousal and drift rate aligns with the primary DDM finding that effort lowered drift rates. This pattern is consistent with a supra-optimal arousal or noise account: higher arousal may introduce noise or resource competition that reduces the efficiency of information accumulation. The lack of strong pupil–bias (z) relationships is consistent with the weak evidence for bias-reset effects in the primary DDM analysis, suggesting that arousal may primarily affect information accumulation rather than response preparation or starting-point adjustments.

## Core DDM Results

### Fixed Effects Overview

The primary model estimated fixed effects for all DDM parameters across difficulty, task, and effort conditions. @fig-fixed-effects-adt and @fig-fixed-effects-vdt show forest plots of fixed effects by task, and @tbl-fixed-effects provides a summary table on the link scale. @tbl-ddm-parameters-natural shows parameter estimates on the natural scale for interpretability.
...
-   **Easy vs. Hard on boundary (a)**: Negative effect (Mean Δ ≈ -0.04 on log scale, or approximately 4% reduction), consistent with reduced caution.
-   **Task differences**: VDT shows systematically different parameter values than ADT, supporting task-specific processing.
-   **Effort on drift and t₀**: High effort shows small but credible effects on information accumulation and motor execution time (t₀ increase of approximately 0.03 log-units or approximately 7.5 ms).

### Bias Estimates (Standard-Only Model)

The Standard-only bias calibration model estimated a negative drift rate on Standard trials (posterior mean v = -1.404, 95% CrI \[-1.662, -1.147\]), consistent with the primary model's estimate (v ≈ -1.26). Starting-point bias was slightly above 0.5 (posterior mean z = 0.567, 95% CrI \[0.534, 0.601\]), indicating a slight bias toward "different" responses, though the strong negative drift dominates the decision process. VDT showed less bias toward "different" than ADT (Δ = -0.179, 95% CrI \[-0.259, -0.101\], P(Δ\>0) \< 0.001). @tbl-bias-levels shows bias levels by condition, @tbl-bias-contrasts shows bias contrasts, and @fig-bias-by-task visualizes bias by task modality.
...
We conducted exploratory analyses examining relationships between LC structural integrity metrics and (i) subject-level pupil indices, and (ii) subject-level DDM parameters and effort-related shifts. LC integrity was quantified using a PCA-based composite (PC1) derived from LC contrast-to-noise ratio (CNR), diffusion metrics (FA, MD), and NODDI metrics (fiso, ficvf). These analyses are explicitly exploratory and complement the primary pupillometry-based arousal measures.

```{r}
#| label: tbl-lc-integrity-models
#| tbl-cap: "Exploratory LC Integrity Models: Associations with DDM Parameters and Pupil Measures"
#| echo: false

lc_models_file <- file.path(proj_root, "output", "lc_integrity", "lc_models_summary.csv")

if (file.exists(lc_models_file)) {
  lc_models <- read_csv(lc_models_file, show_col_types = FALSE)
  
  # Filter to lc_composite term (the main predictor)
  lc_models_display <- lc_models %>%
    filter(term == "lc_composite") %>%
    mutate(
      priority = case_when(
        model_id == "ddm_v_mean" ~ 1,
        model_id == "pupil_phasic_mean_w3" ~ 2,
        str_detect(model_id, "^ddm_") ~ 3,
        str_detect(model_id, "^pupil_") ~ 4,
        TRUE ~ 5
      )
    ) %>%
    arrange(priority) %>%
    head(3) %>%  # Show top 3 key models
    mutate(
      Model = case_when(
        model_id == "ddm_v_mean" ~ "Drift Rate (v)",
        model_id == "pupil_phasic_mean_w3" ~ "Phasic Pupil (W3)",
        model_id == "pupil_v_mean" ~ "Pupil: Drift Rate",
        model_id == "pupil_bs_mean" ~ "Pupil: Boundary Separation",
        model_id == "pupil_z_mean" ~ "Pupil: Bias",
        TRUE ~ str_replace_all(model_id, "_", " ")
      ),
      `Estimate [95% CI]` = sprintf("%.3f [%.3f, %.3f]", estimate, conf.low, conf.high),
      `p-value` = sprintf("%.4f", p.value),
      N = n_obs
    ) %>%
    select(Model, `Estimate [95% CI]`, `p-value`, N)
  
  if (nrow(lc_models_display) > 0) {
    lc_models_display %>%
      gt() %>%
      cols_align(align = "left", columns = Model)
  }
} else {
  cat("*LC exploratory analyses: Run `scripts/exploratory_lc_integrity.R` to generate results.*")
}
```

```{r}
#| label: fig-lc-integrity-cnr-delta-z
#| fig-cap: "Exploratory: LC Contrast-to-Noise Ratio (CNR) vs. Effort-Related Bias Shift. Scatter plot showing the relationship between LC structural integrity (CNR) and the change in starting-point bias between high-effort and low-effort conditions. Each point represents one subject. The regression line with 95% confidence interval (shaded ribbon) shows the exploratory association. This figure examines whether LC integrity moderates effort-related shifts in decision bias."
#| fig-align: center
#| out-width: "90%"
#| echo: false

lc_fig_file <- fig_path("fig_lc_integrity_cnr_vs_delta_z")
# Use include_graphics_safe which handles LaTeX paths correctly
# Check if file exists before including (LC figures may not be generated yet)
if (file.exists(lc_fig_file) || file.exists(file.path("../output/figures", basename(lc_fig_file)))) {
  include_graphics_safe(lc_fig_file)
} else {
  # Figure not found - suppress output in final document
  invisible(NULL)
}
```

```{r}
#| label: fig-lc-integrity-pc1-ddm-v
#| fig-cap: "Exploratory: LC Integrity (PC1) vs. Drift Rate (v). Scatter plot showing the relationship between LC structural integrity (PCA composite) and subject-level drift rate estimates. Each point represents one subject. The regression line with 95% confidence interval (shaded ribbon) shows the exploratory association. This figure examines whether LC integrity relates to individual differences in evidence accumulation efficiency."
#| fig-align: center
#| out-width: "90%"
#| echo: false

lc_fig_file2 <- fig_path("fig_lc_integrity_pc1_vs_ddm_v")
# Use include_graphics_safe which handles LaTeX paths correctly
# Check if file exists before including (LC figures may not be generated yet)
if (file.exists(lc_fig_file2) || file.exists(file.path("../output/figures", basename(lc_fig_file2)))) {
  include_graphics_safe(lc_fig_file2)
} else {
  # Figure not found - suppress output in final document
  invisible(NULL)
}
# If no figure found, chunk returns nothing (no NULL)
```

```{r}
#| label: fig-lc-integrity-pc1-pupil-phasic
#| fig-cap: "Exploratory: LC Integrity (PC1) vs. Phasic Pupil Response. Scatter plot showing the relationship between LC structural integrity (PCA composite) and subject-level phasic pupil response (task-evoked pupil response, W3 window). Each point represents one subject. The regression line with 95% confidence interval (shaded ribbon) shows the exploratory association. This figure examines whether LC integrity relates to individual differences in phasic arousal responses."
#| fig-align: center
#| out-width: "90%"
#| echo: false

lc_fig_file3 <- fig_path("fig_lc_integrity_pc1_vs_pupil_phasic")
# Use include_graphics_safe which handles LaTeX paths correctly
# Check if file exists before including (LC figures may not be generated yet)
if (file.exists(lc_fig_file3) || file.exists(file.path("../output/figures", basename(lc_fig_file3)))) {
  include_graphics_safe(lc_fig_file3)
} else {
  # Figure not found - suppress output in final document
  invisible(NULL)
}
...
MCMC diagnostics indicated acceptable convergence for all parameters (Rhat < 1.01, ESS > 400). Posterior predictive checks (PPC) showed adequate model fit: subject-wise mid-body PPCs showed acceptable error magnitudes (QP RMSE ≤ 0.09 s for most cells; ≤15% flagged), and the model captures central RT tendencies and accuracy well. PPCs were good for Standard and Easy cells (QP RMSE < 0.13, KS < 0.08), with modest misfit in VDT-Hard (worst QP RMSE ≈ 0.206). This pattern suggests some residual fast-tail behavior not captured by a constant-drift Wiener process. Pooled conditional PPCs reveal residual fast-tail misfit, most pronounced in Easy/VDT conditions. This is a known limitation of constant-drift Wiener DDMs without across-trial variability (sv, sz, st₀) or explicit contaminant/lapse processes.

On Standard trials, the model accurately predicted choice proportions: observed 10.9% "Different" responses vs. predicted 11.2% (95% credible interval: [9.9%, 12.7%]). The difference of 0.3% falls well within acceptable ranges, confirming that the model captures the data distribution accurately. Parameter consistency checks confirmed internal consistency: RT asymmetry patterns, drift directions, and individual differences align with model predictions.

Detailed posterior predictive checks and diagnostic figures are provided in Appendix A3 (see @sec-ch3-ppc-method–@sec-ch3-ppc-visual).
...
These findings can be interpreted in the context of LC-NE system dynamics. Recent work suggests that phasic arousal, indexed by pupil dilation, can suppress pre-existing choice biases, "resetting" the decision process to a neutral state [@degee2020pupil]. Consistent with the weak relationship between arousal and starting-point bias observed in our direct pupil–DDM integration analysis (see @sec-pupil-ddm-integration), bias correlations were small and not credibly different from zero (e.g., tonic × z: r = 0.085 [-0.021, 0.190], P(r>0) = 0.941). In our study, the lack of a significant effort effect on bias suggests that the high-effort manipulation (40% MVC) may not have elicited strong enough phasic arousal responses to modulate starting-point bias, or that any such effects were offset by other factors. Alternatively, the integrity of the LC-NE system in our older adult sample may have moderated the expected bias suppression [@huang2024]. The task-specific bias differences (VDT < ADT) may reflect inherent differences in how auditory versus visual detection tasks engage response strategies, independent of arousal state. The direct pupillometry integration confirms that arousal primarily affects information accumulation (drift rate) rather than response preparation or starting-point adjustments in this sample.
...
## S1. Model Diagnostics and Priors
...
## S2. Posterior Predictive Checks (Full Visuals)

Full PPC visual diagnostics are provided in Appendix A3 (@sec-ch3-ppc-visual). See @fig-ch3app-pdiff-heatmap, @fig-ch3app-ppc-small-multiples, @fig-ch3app-ppc-validation, @fig-ch3app-ppc-rt-overlay, and @fig-ch3app-qp-plot.

## S3. Model Sanity Checks (Parameter Consistency)

Full sanity-check figures are provided in Appendix A3 (@sec-ch3-ppc-sanity). See @fig-ch3app-sanity-rt-asymmetry, @fig-ch3app-sanity-hard-drift, and @fig-ch3app-sanity-subject-heterogeneity.

## S4. Individual Differences and Parameter Relationships
...
## S5. Conditional Accuracy Function (CAF)
...
@tbl-ch3app-ppc-gate shows the pooled PPC gate summary (see Appendix A3.6).
...
# Appendix A3: Posterior Predictive Checks and Diagnostics {#app-ch3-ppc}

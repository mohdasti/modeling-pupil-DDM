...
if (!is.null(pupil_data) && "baseline_B0" %in% names(pupil_data) && "task" %in% names(pupil_data) && "effort_condition" %in% names(pupil_data)) {
  ...
  baseline_pupil_summary %>%
    gt(groupname_col = "Task_Group") %>%
    tab_header(title = md("**Baseline Pupil Diameter by Condition**")) %>%
...
if (!is.null(pupil_data) && "cognitive_auc" %in% names(pupil_data) && "task" %in% names(pupil_data) && "effort_condition" %in% names(pupil_data) && "difficulty_level" %in% names(pupil_data)) {
  ...
  tepr_summary %>%
    gt(groupname_col = "Task_Group") %>%
    tab_header(title = md("**Task-Evoked Pupil Response (TEPR) by Condition**")) %>%
...
if (!is.null(trial_sens) && nrow(trial_sens) > 0) {
  ...
  tbl <- tbl_data %>%
    gt::gt() %>%
    gt::tab_header(title = gt::md("**Trial-count sensitivity**")) %>%
    gt::cols_label(
...
if (!is.null(task_strat) && nrow(task_strat) > 0) {
  ...
  tbl <- tbl_data %>%
    gt::gt() %>%
    gt::tab_header(title = gt::md("**Task-stratified correlations**")) %>%
    gt::cols_label(
      relationship = "Relationship",
      task = "Task",
      n_subjects = "N",

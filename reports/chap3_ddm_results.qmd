...
-   **Non-decision time (t₀)** – the duration of processes outside of the evidence accumulation itself, such as perceptual encoding of the stimulus and the motor execution of the response [@voss2004interpreting]. Non-decision time accounts for aspects of the reaction time that are not decision-related.
...
At a neural level, these behavioral patterns reflect underlying changes in the locus coeruleus–norepinephrine (LC–NE) system. The LC–NE system regulates the "gain" of neural networks, enhancing the signal-to-noise ratio of information processing [@astonjones2005; @servan1990]. According to Adaptive Gain Theory, optimal performance relies on a balance between moderate tonic (baseline) arousal and robust phasic (event-related) bursts of norepinephrine [@astonjones2005]. In aging, this system can become dysregulated. While some evidence suggests older adults engage in compensatory upregulation of the LC [@mather2016; @dave2025], this strategy has limits. If the LC–NE system is over-activated by high effort—such as sustaining a 40% MVC handgrip—it may drive the brain into a hyper-aroused state where neural gain is indiscriminate, amplifying noise alongside signal [@eldar2013; @huang2024]. This "neural noise" hypothesis aligns with observations that older adults are particularly vulnerable to distraction and interference under high-arousal conditions [@hanoch2004].

What are the expected effects of arousal fluctuations on the decision-making mechanisms of older adults? By applying the drift-diffusion model (DDM), we can make specific hypotheses about how effort-induced arousal will modulate latent decision parameters. The present study manipulates arousal via physical effort (5% vs. 40% MVC), providing a controlled way to "push" older participants along the arousal curve. Based on the theoretical integration of Resource Competition and Neural Gain Theory, two primary predictions emerge.

First, we anticipate that excessive arousal will degrade the quality of evidence accumulation, manifesting as a reduction in drift rate ($v$) under high-effort conditions (40% MVC) relative to low-effort conditions. Theoretically, if the combination of physical and cognitive demand pushes the older adult onto the descending limb of the arousal-performance curve, the resulting supra-optimal arousal acts as internal noise, reducing the fidelity of evidence accumulation [@eldar2013; @mather2016]. Alternatively, from a Resource Competition perspective, maintaining a 40% MVC grip consumes central processing resources that would otherwise be allocated to the decision process [@azer2023; @wickens2008], directly slowing the rate at which diagnostic information can be extracted. This prediction contrasts with findings in younger adults, where concurrent moderate physical effort (e.g., ~30% MVC) in a visual discrimination task was shown to speed reaction times [@zenon2014], highlighting that younger adults often benefit from arousal boosts that older adults find overwhelming.

Second, we hypothesize that under high arousal, older adults will adopt an even more cautious decision criterion, manifesting as an increase in boundary separation ($a$). If the evidence accumulation process becomes noisier due to arousal (reduced drift), a rational compensatory strategy is to demand more evidence before committing to a choice. Older adults are generally risk-averse and prioritize accuracy; for instance, Starns and Ratcliff [@starns2010] showed that in recognition memory tasks, older adults were reluctant to lower their decision boundaries even when instructed to prioritize speed. We expect this tendency to be exacerbated under stress. While younger adults might respond to arousal with impulsivity, older adults typically respond to challenge by slowing down to preserve accuracy [@ratcliff2004aging]. An increase in threshold ($a$) would suggest that older adults are implicitly aware of the greater uncertainty in their processing under high effort and adapt by trading time for accuracy.Finally, regarding non-decision time ($t_0$) and bias ($z$), predictions are exploratory but grounded in recent findings. It is important to distinguish between concurrent and sequential effects: while Bachman et al. [@bachman2023] found that interleaved isometric exercise (performed between blocks of an auditory working memory task) speeded reaction times in older adults, our paradigm requires concurrent exertion. The dual-task cost of maintaining the grip during the decision may offset any arousal-related motor facilitation, potentially resulting in a net increase or null effect on $t_0$ [@azer2023]. Regarding bias ($z$), recent work using perceptual detection tasks suggests that phasic arousal, indexed by pupil dilation, can suppress pre-existing choice biases, "resetting" the decision process to a neutral state [@deGee2020pupil]. If high effort elicits strong phasic responses, we might observe a reduction in decision bias ($z$ approaching 0.5), although this may be moderated by the integrity of the individual's LC-NE system [@huang2024].

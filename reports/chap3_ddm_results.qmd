...
for (path in pupil_data_paths) {
  if (file.exists(path)) {
    tryCatch({
      ...
      # Map cognitive_auc column name (cog_auc or cognitive_auc)
      if ("cog_auc" %in% names(pupil_data) && !"cognitive_auc" %in% names(pupil_data)) {
...
if (!is.null(pupil_data) && "cognitive_auc" %in% names(pupil_data) && "task" %in% names(pupil_data) && "effort_condition" %in% names(pupil_data) && "difficulty_level" %in% names(pupil_data)) {
  ...
  tepr_summary %>%
    ...
    tab_footnote(
      footnote = "TEPR (phasic arousal) quantified as Cognitive AUC (Area Under the Curve) from baseline-corrected pupil data (4.65s to response onset). AUC in units of mm·s.",

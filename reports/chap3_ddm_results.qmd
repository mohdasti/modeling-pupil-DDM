...
if (!is.null(bias_contr_std)) {
  bias_contr_std %>%
    mutate(across(where(is.numeric), ~round(.x, 3))) %>%
    select(contrast, mean, q2.5, q97.5, Pr_gt_0) %>%
    gt() %>%
...
if (!is.null(sensitivity_summary)) {
  sens_subchance <- sensitivity_summary %>%
    ...
    select(model, parameter, baseline_estimate, sensitivity_estimate, delta, 
...
**Results**: All key parameters showed robust estimates when excluding sub-chance participants. For Model3_Difficulty, the drift intercept, boundary separation, and difficulty effects (Hard, Easy) all showed delta CIs that included zero, indicating no meaningful change in parameter estimates. The same pattern held for Model4_Additive. **Conclusion**: The inclusion of sub-chance participants did not alter main effects, supporting our decision to retain all 67 participants to maximize sample size and leverage hierarchical modeling's ability to stabilize estimates through shrinkage.

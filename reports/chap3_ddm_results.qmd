...
The cognitive AUC was calculated from **300ms after target stimulus onset until 3.3s post-target** (4.65s to 7.65s relative to squeeze onset), using baseline-corrected pupil data. Baseline correction was performed by subtracting the mean pupil diameter in the baseline window (-0.5s to 0s relative to squeeze onset) from all timepoints in the trial. AUC was computed using trapezoidal integration of the baseline-corrected pupil signal within the cognitive window. This window (W3.0) was selected to capture the full task-evoked pupil response (TEPR) peak, which occurs at ~2.95s post-target. 
...
### Sensitivity Analysis and Confound Mitigation

To address potential concerns about post-response contamination and motor execution confounds, we implemented multiple mitigation strategies:

1. **Early Window (W1.3)**: We computed an early cognitive window (W1.3: target+0.3 to target+1.3s; 4.65s to 5.65s relative to squeeze onset) that extends only 0.95s into the response period. This window yields two complementary metrics: `cog_auc_w1p3` (area under the curve in the W1.3 window) and `cog_mean_w1p3` (mean pupil diameter in the W1.3 window). These early-window features provide a more conservative estimate of pre-response cognitive processing, as they capture the initial phase of the TEPR with minimal contamination from post-response processes.

2. **RT as Covariate**: For trial-level DDM-pupil coupling models (when implemented), RT should be included as a covariate to control for decision state and prevent RT from confounding pupil-DDM relationships. This is critical because RT correlates with both decision processes and pupil dynamics during the response period. Note: The current analyses use subject-level correlations between DDM parameters and pupil measures, which aggregate across trials and thus naturally account for RT variability at the subject level.

3. **Motor Buffer Truncation** (available for future analyses): Window definitions are computed to truncate W3.0 at 150ms before button press (`t_resp - 0.15s`) to minimize motor execution/blink contamination. Full AUC computation for motor-buffered windows requires re-processing flat files with RT information.
...
LC MRI metrics computed by colleagues in our research group were used in this chapter. The methodological approach follows [@bennett2024lc], who describe a pipeline for deriving LC contrast from magnetization transfer (MT) imaging and extracting diffusion metrics within LC regions of interest (ROIs). Specifically, [@bennett2024lc] compute an MTC (magnetization transfer contrast) map relative to a pontine reference region and extract LC ROI summary values (average/maximum contrast). Diffusion metrics (DTI/NODDI-style metrics) were extracted within the LC ROI following the diffusion preprocessing approach described in [@bennett2024lc].
...
These LC integrity metrics are used in exploratory individual-differences analyses to examine whether LC structural properties relate to (i) subject-level pupil indices (baseline diameter, TEPR magnitude), and (ii) subject-level DDM parameters and effort-related shifts (especially bias shift), as exploratory moderation/association analyses. These analyses are explicitly exploratory and are not part of the primary confirmatory hypothesis testing framework.
...
Prior vs. posterior for t0 is shown in Supplementary Figure @fig-ndt-prior-posterior.
...
The full LOO comparison plot is provided in Supplementary Figure @fig-loo.

...
if (!is.null(behavioral_data) && !is.null(rt_col) && !is.null(task_col) && !is.null(diff_col) && !is.null(effort_col)) {
  ...
  if (nrow(rt_plot_data) > 0) {
    ...
    cat("RT distribution plot: Data not available. Please ensure behavioral data contains RT values.\n")
  }
} else {
  cat("RT distribution plot: Behavioral data or RT column not found. Placeholder figure will be generated when data is available.\n")
}
```
...
```{r, fig.cap="Task-Evoked Pupil Response (TEPR) Time Course. Grand-average pupil traces (mean ± 95% CI) aligned to response signal onset for Low effort (blue) and High effort (crimson) conditions, separately for ADT (left) and VDT (right). The shaded region indicates the time window used for TEPR quantification. Baseline pupil (pre-stimulus) is shown in the -1 to 0 s window. This figure will be generated once pupillometry analysis is complete.", out.width="95%", fig.align="center"}

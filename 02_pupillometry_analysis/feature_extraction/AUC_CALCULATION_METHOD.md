...
2. **`cognitive_auc`**: Cognitive AUC (primary metric for cognitive TEPR)
   ...
   - **QC Metrics** (if using `calculate_auc_with_qc()`):
     ...
     - `cog_auc_max_gap_ms`: Maximum contiguous gap in cognitive window (ms)
...
### Chapter 3 (DDM with Pupil Predictors - Modeling-Friendly)

**Primary**:
- `baseline_quality >= 0.50` AND `cog_quality >= 0.50`
- AND `cog_auc_max_gap_ms <= 250`
- AND RT filter: `0.2s <= RT <= 3.0s`

**Rationale**: DDM analyses benefit from larger sample sizes, so we use a more lenient quality threshold (50%) while still enforcing gap-aware exclusions to prevent distorted AUC estimates.

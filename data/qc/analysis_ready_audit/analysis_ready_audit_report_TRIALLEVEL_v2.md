...
PASS requires:
- Median trials per subject×task >= 120 (observed: 27.5 ❌)
- Median trials per run >= 25 (observed: 11 ❌)
- >=90% of (subject×task) have all 5 runs (observed: 29.76% ❌)
- Gate bias by task <= 10pp (observed: 14.3pp ❌)
...
- **Subject×task with all 5 runs:** 25 / 84  ( 29.76 %) ❌ (target: >=90%)
...
- **Max difference:** 14.3 percentage points ❌ (threshold: <=10pp)
...
|-----------|------|--------|---------------|-----------|------|------|
| triallevel | BAP_analysis_ready_TRIALLEVEL.csv | Yes | 2025-12-20 00:44:56 | 0.76 | 3357 | 31 |

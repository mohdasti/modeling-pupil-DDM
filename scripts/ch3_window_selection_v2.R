#!/usr/bin/env Rscript
# ============================================================================
# Chapter 3 Window Selection and Timing Verification (v2 - using waveform summaries)
# ============================================================================
...
cat("=== Chapter 3 Window Selection (v2) ===\n\n")
...
# CH3 EXTENSION: Expected data extends to ~7.7s from squeeze (~3.35s from target)
...
# CH3 EXTENSION: Verify data extends to at least 7.65s from squeeze (target + 3.3s minimum)
...
waveform_by_task <- waveform_data %>%
...
waveform_by_effort <- waveform_data %>%
...
waveform_by_oddball <- waveform_data %>%
...
p_task <- ggplot(waveform_by_task, aes(x = t_from_target, y = mean_pupil, color = task)) +
  ...
  geom_line(size = 1) + geom_vline(xintercept = 0, linetype = "dashed", color = "gray50") +
...
# PART 3: Window selection
cat("PART 3: Window Selection Diagnostics\n")
cat("------------------------------------\n")

# CH3 EXTENSION: Peak search window: 0.3 to 3.3 seconds post-target
...
# CH3 EXTENSION: Window coverage - W3.0 now feasible with Resp1ET extension
ch3_data <- fread(CH3_FILE)
windows <- list(
  W2.0 = c(0.3, 2.3),  # target+0.3 to target+2.3
  W2.5 = c(0.3, 2.8),  # target+0.3 to target+2.8
  W3.0 = c(0.3, 3.3)   # target+0.3 to target+3.3 (now feasible with Resp1ET extension)
)
window_coverage_list <- list()

for (win_name in names(windows)) {
  win_start <- windows[[win_name]][1] + EXPECTED_TARGET_ONSET
  win_end <- windows[[win_name]][2] + EXPECTED_TARGET_ONSET
  
  # For waveform data, we can estimate coverage from available time range
  # In practice, this would need trial-level validation
  coverage_by_task <- waveform_data %>%
    group_by(task) %>%
    summarise(
      has_data_in_window = any(t_rel >= win_start & t_rel <= win_end),
      .groups = "drop"
    ) %>%
    mutate(window = win_name, window_start_rel = win_start, window_end_rel = win_end)
  
  window_coverage_list[[win_name]] <- coverage_by_task
}

window_coverage <- bind_rows(window_coverage_list) %>%
  group_by(window, window_start_rel, window_end_rel, task) %>%
  summarise(
    has_coverage = any(has_data_in_window),
    .groups = "drop"
  ) %>%
  mutate(
...
recommended_window <- window_coverage %>%
  filter(window_end_post_target >= max_peak_time + 0.2) %>%
  arrange(window_end_post_target) %>%
  slice(1)

if (nrow(recommended_window) == 0) {
  recommended_window <- window_coverage %>% arrange(desc(window_end_post_target)) %>% slice(1)
}

cat("\nRecommended window:", recommended_window$window, "\n")
cat("  Window: target +", recommended_window$window_start_post_target, "to target +", recommended_window$window_end_post_target, "seconds\n")

recommendation_text <- paste0(
  "# Chapter 3 Cognitive Window Recommendation\n\n",
  "## Recommended Window\n",
  "- **Window Name**: ", recommended_window$window, "\n",
  "- **Time Range**: target onset + ", recommended_window$window_start_post_target, "s to target onset + ", recommended_window$window_end_post_target, "s\n",
  "- **Maximum peak time**: ", round(max_peak_time, 2), "s\n\n"
)

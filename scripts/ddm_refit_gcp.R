# =========================================================================
# DDM REFITTING WITH NEW RT THRESHOLD (GCP VERSION)
# =========================================================================
# Optimized for RStudio Cloud on GCP. Run check_vm_capabilities.R first
# to inspect CPU/RAM and tune DDM_N_CHAINS / DDM_THREADS_PER_CHAIN.
#
# RT Definitions:
...
#
# GCP env vars:
#   DDM_GCP_PROJECT_ROOT  - Project root (default: inferred from script path)
#   DDM_N_CHAINS          - MCMC chains (default: auto from detectCores())
#   DDM_THREADS_PER_CHAIN - Threads per chain (default: auto)
# =========================================================================

# -------------------------------------------------------------------------
# GCP SETUP: Set working directory to project root
# -------------------------------------------------------------------------
# Set DDM_GCP_PROJECT_ROOT explicitly if running from RStudio Cloud
...
if (nchar(PROJECT_ROOT) > 0 && dir.exists(PROJECT_ROOT)) {
  setwd(PROJECT_ROOT)
  message("[GCP] Set working directory to: ", getwd())
} else {
  message("[GCP] Using current directory: ", getwd())
}
...
# MCMC settings (GCP: auto-detect or use env vars)
DETECTED_CORES <- parallel::detectCores()
n_chains_env <- Sys.getenv("DDM_N_CHAINS", unset = "")
N_CHAINS <- if (nchar(n_chains_env) > 0) {
  suppressWarnings(max(2, min(as.integer(n_chains_env), DETECTED_CORES)))
} else {
  min(4, max(2, DETECTED_CORES - 2))
}
threads_env <- Sys.getenv("DDM_THREADS_PER_CHAIN", unset = "")
THREADS_PER_CHAIN <- if (nchar(threads_env) > 0) {
  suppressWarnings(max(1, min(as.integer(threads_env), 4)))
} else {
  max(1, floor((DETECTED_CORES - N_CHAINS) / N_CHAINS))
}
N_ITER <- 2000
N_WARMUP <- 1000
ADAPT_DELTA <- 0.95
...
log_info(paste("Number of cores:", DETECTED_CORES))
log_info(paste("N_CHAINS:", N_CHAINS))
log_info(paste("THREADS_PER_CHAIN:", THREADS_PER_CHAIN))
...
# -------------------------------------------------------------------------
# TASK-SENSITIVITY MODEL: Task-varying drift and boundary (peer-review response)
# -------------------------------------------------------------------------
# Rationale: Peer reviewers noted that excluding Task from drift rate (v) and
# boundary separation (a) is psychophysically implausible. This sensitivity
# model tests whether: (1) Task modality affects processing speed, (2) Effort x
# Task interaction exists, (3) The core Effort effect on drift survives.
# Drift: difficulty + effort * task (with interaction)
# Boundary: difficulty + effort + task (main effects only, for stability)
# NDT and bias: unchanged from primary model
# -------------------------------------------------------------------------
bs_task_sens_formula <- if (USE_BS_RANDOM_EFFECTS) {
  as.formula("bs ~ 1 + effort_condition + task + difficulty_3 + (1 | subject_id)")
} else {
  as.formula("bs ~ 1 + effort_condition + task + difficulty_3")
}
formula_task_sensitivity <- bf(
  rt | dec(choice_binary) ~ 1 + effort_condition * task + difficulty_3 + (1 | subject_id),
  bs_task_sens_formula,
  ndt ~ 1,
  bias ~ task + effort_condition + (1 | subject_id)
)

# -------------------------------------------------------------------------
# PUPIL INTERACTION MODEL: pupil_scaled * difficulty (advisor request)
# -------------------------------------------------------------------------
# Tests whether trial-by-trial arousal (pupil_scaled) effect on drift and
# boundary differs by cognitive load (difficulty). NDT and bias unchanged.
# Requires pupil data (ddm_pupil_ready_thr0.20_probe_onset_locked.csv).
# -------------------------------------------------------------------------
bs_pupil_interaction_formula <- if (USE_BS_RANDOM_EFFECTS) {
  as.formula("bs ~ 1 + effort_condition + difficulty_3 + pupil_scaled * difficulty_3 + (1 + pupil_scaled * difficulty_3 | subject_id)")
} else {
  as.formula("bs ~ 1 + effort_condition + difficulty_3 + pupil_scaled * difficulty_3")
}
formula_pupil_interaction <- bf(
  rt | dec(choice_binary) ~ 1 + effort_condition + difficulty_3 + pupil_scaled * difficulty_3 + (1 + pupil_scaled * difficulty_3 | subject_id),
  bs_pupil_interaction_formula,
  ndt ~ 1,
  bias ~ task + effort_condition + (1 | subject_id)
)
...
cat("\n  Pupil interaction model (when pupil data available):\n")
cat("    Drift (v): rt | dec(choice_binary) ~ 1 + effort_condition + difficulty_3 + pupil_scaled * difficulty_3 + (1 + pupil_scaled * difficulty_3 | subject_id)\n")
cat("    Boundary (bs): bs ~ 1 + effort_condition + difficulty_3 + pupil_scaled * difficulty_3 + (1 + pupil_scaled * difficulty_3 | subject_id)\n")
cat("    NDT (t0): ndt ~ 1\n")
cat("    Bias (z): bias ~ task + effort_condition + (1 | subject_id)\n\n")
...
# Fit task-sensitivity model (Task in drift and boundary; Effort × Task interaction on drift)
# Peer reviewers: psychophysically implausible to exclude Task from drift/boundary
cat("\n")
log_info("Fitting task-sensitivity model (Task in drift + boundary, Effort×Task on drift)...")
task_sensitivity_probe <- fit_model(
  formula_task_sensitivity, ddm_data_primary_probe, rt_map_probe$rt_col,
  "task_sensitivity", rt_map_probe$rt_type, use_predictors = TRUE, threshold = CHOSEN_THRESHOLD
)

# -------------------------------------------------------------------------
# PUPIL INTERACTION MODEL (optional: requires pupil data)
# -------------------------------------------------------------------------
PUPIL_DATA_FILE <- file.path("output", "ddm_pupil", paste0("ddm_pupil_ready_thr", formatC(CHOSEN_THRESHOLD, format = "f", digits = 2), "_probe_onset_locked.csv"))
if (file.exists(PUPIL_DATA_FILE) && !DRY_RUN) {
  cat("\n")
  log_info("Fitting pupil interaction model (pupil_scaled * difficulty on drift and boundary)...")
  ddm_pupil_raw <- read_csv(PUPIL_DATA_FILE, show_col_types = FALSE)
  # Prepare: align with ddm_refit_gcp (task, effort_condition, difficulty_3, pupil_scaled)
  task_col <- na.omit(intersect(c("task", "task_std"), names(ddm_pupil_raw)))[1]
  diff_col  <- na.omit(intersect(c("difficulty_3", "difficulty_level"), names(ddm_pupil_raw)))[1]
  log_info(paste("Pupil data: task col =", if (is.na(task_col)) "NONE" else task_col,
                 " | diff col =", if (is.na(diff_col)) "NONE" else diff_col))
  if (!is.na(task_col)) {
    log_info(paste("Pupil data unique task values:", paste(unique(ddm_pupil_raw[[task_col]]), collapse = ", ")))
  }
  ddm_pupil_fit <- ddm_pupil_raw %>%
    mutate(
      rt = rt_primary,
      # Normalize task to ADT/VDT regardless of original coding (aud/vis/ADT/VDT/1/2)
      task = {
        raw_task <- if (!is.na(task_col)) tolower(as.character(.data[[task_col]])) else "adt"
        factor(
          case_when(
            grepl("^a|adt|aud", raw_task) ~ "ADT",
            grepl("^v|vdt|vis", raw_task) ~ "VDT",
            raw_task == "1" ~ "ADT",
            raw_task == "2" ~ "VDT",
            TRUE ~ NA_character_
          ),
      ...
      difficulty_3 = factor(
        case_when(
          grepl("standard|baseline|^0$", as.character(.data[[diff_col]]), ignore.case = TRUE) ~ "Standard",
          grepl("easy|^3$|^4$", as.character(.data[[diff_col]]), ignore.case = TRUE) ~ "Easy",
          grepl("hard|^1$|^2$", as.character(.data[[diff_col]]), ignore.case = TRUE) ~ "Hard",
          TRUE ~ "Standard"
        ),
    ...
    filter(!is.na(rt), rt > CHOSEN_THRESHOLD, rt < 3.0, choice_binary %in% c(0L, 1L),
           !is.na(pupil_scaled), !is.na(task))
  log_info(paste("Pupil model data: N =", nrow(ddm_pupil_fit), "trials, task levels:",
                 paste(levels(ddm_pupil_fit$task), collapse = "/")))
  rt_map_pupil <- list(rt_col = "rt", rt_type = "probe_onset_locked")
  pupil_interaction_probe <- fit_model(
    formula_pupil_interaction, ddm_pupil_fit, rt_map_pupil$rt_col,
    "pupil_interaction", rt_map_pupil$rt_type, use_predictors = TRUE, threshold = CHOSEN_THRESHOLD
  )
  log_info(paste("Pupil interaction model: N =", nrow(ddm_pupil_fit), "trials"))
} else {
  if (!file.exists(PUPIL_DATA_FILE)) {
    log_info("Skipping pupil interaction model (pupil data not found:", PUPIL_DATA_FILE, ")")
  }
}
...
if (!DRY_RUN) {
  ...
  # -------------------------------------------------------------------------
  # TASK-SENSITIVITY MODEL: Extract and compare with primary model
  # -------------------------------------------------------------------------
  if (exists("task_sensitivity_probe") && !is.null(task_sensitivity_probe) &&
      inherits(task_sensitivity_probe, "brmsfit") &&
      exists("additive_probe") && !is.null(additive_probe) &&
      inherits(additive_probe, "brmsfit")) {
    log_info("Extracting task-sensitivity comparison...")

    fixef_primary <- brms::fixef(additive_probe, summary = TRUE)
    fixef_task_sens <- brms::fixef(task_sensitivity_probe, summary = TRUE)

    # brms uses effort_conditionhigh, taskVDT, effort_conditionhigh:taskVDT (factor levels: low/high, ADT/VDT)
    eff_term <- intersect(
    ...
    task_comparison <- tibble(
      Parameter = c("Effort → Drift (Primary)", "Effort → Drift (Task-Sens)",
                    "Task → Drift", "Effort × Task → Drift"),
      Estimate = c(
        if (!is.na(eff_term)) fixef_primary[eff_term, "Estimate"] else NA_real_,
        if (!is.na(eff_term) && eff_term %in% rownames(fixef_task_sens))
          fixef_task_sens[eff_term, "Estimate"] else NA_real_,
        if (!is.na(task_term)) fixef_task_sens[task_term, "Estimate"] else NA_real_,
        if (!is.na(int_term)) fixef_task_sens[int_term, "Estimate"] else NA_real_
      ),
      CI_lower = c(
        if (!is.na(eff_term)) fixef_primary[eff_term, "Q2.5"] else NA_real_,
        if (!is.na(eff_term) && eff_term %in% rownames(fixef_task_sens))
          fixef_task_sens[eff_term, "Q2.5"] else NA_real_,
        if (!is.na(task_term)) fixef_task_sens[task_term, "Q2.5"] else NA_real_,
        if (!is.na(int_term)) fixef_task_sens[int_term, "Q2.5"] else NA_real_
      ),
      CI_upper = c(
        if (!is.na(eff_term)) fixef_primary[eff_term, "Q97.5"] else NA_real_,
        if (!is.na(eff_term) && eff_term %in% rownames(fixef_task_sens))
          fixef_task_sens[eff_term, "Q97.5"] else NA_real_,
        if (!is.na(task_term)) fixef_task_sens[task_term, "Q97.5"] else NA_real_,
        if (!is.na(int_term)) fixef_task_sens[int_term, "Q97.5"] else NA_real_
      )
    )

    task_sens_file <- file.path(TABLES_DIR, "task_sensitivity_comparison.csv")
    write_csv(task_comparison, task_sens_file)
    log_info(paste("Saved:", task_sens_file))

    cat("\n=== TASK-SENSITIVITY COMPARISON ===\n")
    print(task_comparison)
  } else {
    log_warn("Task-sensitivity comparison skipped (model not fitted or DRY_RUN)")
  }
}
